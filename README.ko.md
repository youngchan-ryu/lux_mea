...
클라우드 엔진은 키가 하나 더 필요하다. `--engine groq`은 `GROQ_API_KEY`,
`--engine clova` / `--engine clova-stream`은 `CLOVA_SPEECH_SECRET`.
...
실행 중에는 `e` 키로 음성 엔진을 바꾼다. `--engine`과 `--fallback-engine` 사이를
오간다. 클라우드 엔진이 네트워크를 잃으면 알아서 로컬로 넘어가고 그 사실을 HUD에
표시하므로, 전시장 와이파이가 죽어도 시연이 아니라 글자 한 줄이 바뀔 뿐이다.
...
| 모델 | 적중률 (1회차 / 2회차) | 지연 시간(중앙값) |
|---|---|---|
| large-v3-turbo | 70% / 40% | 0.49 s |
| **medium** | **80% / 80%** | **0.38 s** |
| small | 60% / 70% | 0.14 s |

`medium`이 기본값이다. 두 번 돌려 같은 점수가 나온 유일한 모델이고, 큰 모델 둘 중에는
더 빠르다. 다만 평균이 아니라 두 열을 그대로 볼 것 — 같은 조건에서 같은 모델이 70%와
40%를 오갔다. 클립 10개로는 이 차이를 가릴 수 없다. 페어드 검정으로 p ≈ 0.25, 즉
**모른다**는 뜻이다. 이 실험이 분명하게 보여준 것은 순위가 아니라 *실패 방식*이었다:
큰 모델은 조용한 구간에서 같은 문구를 길게 반복하는 환각을 냈고, 그래서 지금은 잡음
바닥을 넘지 못한 구간을 아예 디코더로 보내지 않는다. 원본 출력은
[`bench_asr_result.md`](bench_asr_result.md)에 있다.

`bench_asr.py`는 그 교훈을 반영해 다시 만들었다. 클립 수를 늘리고, 소음 조건을
오프라인에서 섞어 모든 엔진이 완전히 같은 오디오를 듣게 하고, 디코더 프롬프트를 기본으로
켰으며(예전 벤치는 `app.py`가 실제로 쓰는 설정과 다른 설정을 재고 있었다), 비율 두 개를
눈으로 비교하는 대신 `bench_report.py`가 페어드 통계를 낸다.

위 내용은 전부 로컬 실행 기준이다. 시끄러운 환경을 위한 클라우드 엔진이 둘 있다.
`--engine groq`, 그리고 CLOVA Speech를 `--engine clova`(다른 엔진처럼 세그먼트 단위)
또는 `--engine clova-stream`(연결을 열어둔 채 서버가 발화 끝을 판단)으로 쓸 수 있다.
마지막 것이 흥미로운 쪽이다 — 부위 이름을 말한 순간과 빔이 움직이는 순간 사이에서
0.3초 무음 대기와 세그먼트 전체 추론을 통째로 걷어낸다. 그게 청중이 실제로 체감하는
지연이고, 클립 단위 벤치로는 볼 수 없는 값이다. `bench_asr.py live`가 녹음된 발표를
실시간 속도로 파이프라인에 흘려 그 값을 잰다.
